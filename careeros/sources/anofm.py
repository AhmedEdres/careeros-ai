...
Romanian job-mediation platform.  This adapter deliberately treats ANOFM as a
candidate-discovery source, not as a generic company scraper: it pulls a
bounded window of current public postings, keeps the candidate's requested
geography, and preserves the structured requirements that the matching engine
needs (Romanian/English, education, experience, driving licence, EU eligibility,
contract and salary).
"""
...
# The public UI calls the home county "Timiș"; accept the common spellings
# without making the provider depend on a hard-coded numeric county id.
...
def _rows(payload: Any) -> List[Dict[str, Any]]:
    """Accept the common datagrid shapes used by ANOFM deployments."""
    ...
    # Some wrappers nest the datagrid under a result/payload object.
...
def _matches_location(location: str, requested: str) -> bool:
    ...
    if not loc_norm:
        # Do not throw away a structured ANOFM record merely because the API
        # changed the display field name. The ranking layer can still inspect
        # the raw description.
        return True
    if "timisoara" in requested_norm or "timis" in requested_norm:
        return contains_any(loc_norm, _TIMISOARA_TERMS)
    # For another explicit city, require the city token rather than accepting
    # every job in the county/country.
...
def _parse_job(item: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return make_job(
        ...
        salary_min=float(salary_min) if str(salary_min).replace('.', '', 1).isdigit() else None,
        salary_max=float(salary_max) if str(salary_max).replace('.', '', 1).isdigit() else None,
        salary_currency="ron",
...
def fetch_anofm(
    ...
    **_,
    ...
    """Fetch a bounded live window from ANOFM and filter it locally.

    The source API is a datagrid rather than a conventional keyword-search
    API. We therefore fetch several recent pages once, preserve structured
    requirements, then apply the same phrase/domain filter used by the other
    whole-board providers. This avoids one HTTP request per career query.
    """
    source = "ANOFM / Mediere"
